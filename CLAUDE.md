...
- **Pinch-to-zoom is applied at the camera, not in the preprocessing.** `detectTransformGestures` on the overlay feeds a scale delta to `Scanner.zoomBy`, which calls `CameraControl.setZoomRatio` on Android and `AVCaptureDevice.videoZoomFactor` on iOS. Both zoom the *sensor output*, so `ImageAnalysis`/the video data output deliver already-zoomed frames and the crop-and-resize chain is untouched. **Zooming therefore changes what the model sees** - useful for filling the viewfinder with a distant animal. The range is the device's own, capped at `ZOOM_RATIO_CEILING` (8x); a lens whose max is 1.0 (many front cameras) reports `canZoom == false` and both the gesture and the chip disappear. Tapping the zoom chip resets to minimum.
